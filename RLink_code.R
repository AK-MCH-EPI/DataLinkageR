...
# Data Source: All data is fictitious
...
#Read in data sets to me linked
dat1<-read.csv("D:\\Presentations\\R_Linkage\\DataSetA.csv",header=T)
dat2<-read.csv("D:\\Presentations\\R_Linkage\\DataSetB.csv",header=T)
...
#Load in dataset with the actual truth of the linkages specified 
  Truth<-read.csv("D:\\Presentations\\R_Linkage\\MatchTruthFull.csv",header=T)
...
## Deterministic Linkage with no cleaning: ####
  ...
  #Merge with dplyr (as an example of another way to do a merge)
    require(dplyr)
    test1<-inner_join(dat1,dat2,by=c("dob","lname","fname","sex"))
      length(rownames(test1))
      remove(test1)
...
## Probabalistic aka Fuzzy matching ####
...
# Impact of usign different thresholds
...
l2a<-compare.linkage(dat1c,dat2c,strcmp=TRUE,strcmpfun = jarowinkler,exclude = c(1,5,7,8))
  ...
  #Assign weights based on teh strcomp of elements
  e2a<-epiWeights(l2a) 
  ...
  #Can we do better:
    ...
    # How to avoid matching the 6 false positives
  ...
  #Need to respecify for manual review to avoid matching the twins and reduce manual reviews.
  ...
  #create indicator variable for designating match
    pairs.1cm$is_match<-with(pairs.1cm,ifelse(Weight=="1.0000000",1,''))
      View(pairs.1cm)
  #Conduct Manual Reivew and update indicator to 1 if true for all weights between 0.85-0.99
    pairs.1cm<-edit(pairs.1cm)

  #saveRDS(pairs.1cm,"D:\\Presentations\\R_Linkage\\pairs.1cm.RData")
    pairs.1cm<-readRDS("D:\\Presentations\\R_Linkage\\pairs.1cm.RData")
    
    #Work with R a bit to organize these data.
    #remove blank lines and NA rows
    pairs.1cm<-pairs.1cm[rowSums(pairs.1cm=="")!=ncol(pairs.1cm), ]
      pairs.1cm<-pairs.1cm[complete.cases(pairs.1cm), ]
      #Split data into two frames
    pairs.1cm.a<-subset(pairs.1cm,Weight=='')
    pairs.1cm.b<-subset(pairs.1cm,Weight!='')
...
p<-compare.linkage(sub.a,sub.b,strcmp=TRUE,strcmpfun = jarowinkler,exclude = c(1,5,7))
  p.1<-editMatch(p)
    #saveRDS(p.1,"H:\\Presentations\\R_Linkage\\p.1.RData")
    p.1<-readRDS("D:\\Presentations\\R_Linkage\\p.1.RData")
